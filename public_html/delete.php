...
if ( $direction == 'delete' ) {
	...
	$response = Posts::moveFile($file, $kenmerk, IniSettings::get('settings', 'attachment_directory'), IniSettings::get('settings', 'deleted_attachment_directory'));
} elseif ( $direction == 'undelete' ) {
	// UNdelete file (move from deleted directory to attachment direcotry)
	$response = Posts::moveFile($file, $kenmerk, IniSettings::get('settings', 'deleted_attachment_directory'), IniSettings::get('settings', 'attachment_directory'));
}
